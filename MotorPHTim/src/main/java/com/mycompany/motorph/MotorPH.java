...
System.exit(0); }
        ...
        for (int month = 6; month <= 12; month++) {
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(ATT_FILE))) {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    double hours = computeHours(loginTime, logoutTime);
            ...
            System.out.printf("  1st Cutoff (1-15)   : %s hrs%n", formatVal(firstHalf));
            System.out.printf("  2nd Cutoff (16-end) : %s hrs%n", formatVal(secondHalf));
            System.out.printf("  Monthly Total       : %s hrs%n", formatVal(firstHalf + secondHalf));
        }
        System.out.println("\n===================================");
    }
    ...
    public static void calculateSalary(String empNo, String firstName, String lastName,
        ...
        for (int month = 6; month <= 12; month++) {
            ...
            // Rule 5: combine both cutoffs first, then compute deductions
            ...
            // Split deductions: half SSS, full PhilHealth share, half Pag-IBIG per cutoff; tax on 2nd
            ...
            System.out.printf("Total Hours Worked : %s hrs%n", formatVal(firstHalf));
            ...
            System.out.printf("Total Hours Worked : %s hrs%n", formatVal(secondHalf));
    ...
    public static double computeHours(LocalTime loginTime, LocalTime logoutTime) {
        ...
        if (logoutTime.isAfter(endOfWork)) logoutTime = endOfWork;

        // Grace period: if login is 8:10 AM or earlier, treat as exactly 8:00 AM
        if (!loginTime.isAfter(graceTime)) loginTime = startOfWork;

        // Must log in before 5:00 PM and log out after 8:00 AM
        if (!loginTime.isBefore(endOfWork)) return 0;
        if (!logoutTime.isAfter(startOfWork)) return 0;
    ...
    public static String formatVal(double value) {
